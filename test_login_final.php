...
// Script para probar el login con Sanctum - Versión final
echo "=== PRUEBA FINAL DE LOGIN CON SANCTUM ===\n\n";

// URL base
$baseUrl = 'http://kampus.test';

// Archivo para cookies
$cookieFile = 'cookies_final.txt';
...
// 2. Leer el token CSRF de las cookies
echo "2. Leyendo token CSRF de cookies...\n";
$xsrfToken = null;
if (file_exists($cookieFile)) {
    $cookies = file_get_contents($cookieFile);
    if (preg_match('/XSRF-TOKEN\\s+([^\\s]+)/', $cookies, $matches)) {
        $xsrfToken = urldecode($matches[1]);
        echo "✅ Token CSRF encontrado: " . substr($xsrfToken, 0, 20) . "...\n";
    } else {
        echo "❌ No se encontró el token CSRF en las cookies\n";
        echo "Contenido del archivo de cookies:\n$cookies\n";
    }
} else {
...
// Agregar el header X-XSRF-TOKEN si se encontró
if ($xsrfToken) {
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Content-Type: application/json',
        'Accept: application/json',
    ...
    echo "⚠️  No se pudo agregar header X-XSRF-TOKEN\n";
}
